...
class RateEngine
{
	...
	*/
	...
	function rate_engine_calculcost (&$A2B, $callduration, $K=0)
	{
		...
		if ($this -> debug_st)  echo "CALLDURATION: $callduration - freetimetocall_left: ".$this->freetimetocall_left[$K]." - freetimetocall_used: $this->freetimetocall_used\n\n";
		$A2B -> debug( DEBUG, $agi, __FILE__, __LINE__, "[CC_RATE_ENGINE_CALCULCOST: K=$K - CALLDURATION:$callduration - freetimetocall_left:".$this->freetimetocall_left[$K]." - freetimetocall_used:$this->freetimetocall_used]");
		...
		if (empty($chargea) || $chargea==0 || empty($timechargea) || $timechargea==0) {
			...
			if ($this -> freetimetocall_left[$K] >= $callduration) {
				...
				$callduration -= $this -> freetimetocall_left[$K];
			}
	...
	*/
	...
	function rate_engine_updatesystem (&$A2B, &$agi, $calledstation, $doibill = 1, $didcall=0, $callback=0)
	{
		...
		if ($sessiontime > 0) {
			...
			if (($id_cc_package_offer!=-1) && ($this ->package_to_apply[$K] != null) && ($sessiontime >= $A2B->agiconfig['min_duration_2bill'])) {
